...
//	private static String theStartStationDataFile = "xml/"+scenarioFolder+"/startstation.xml"; 
	...
	/**
	 * set the file paths for all of the files currently hard coded
	 */
	...
	private static void createWellenGenerator() {
		...
		try {
			...
			for (Element theWellengenerator : wellengeneratoren) {
				...
				// create the inqueue
				SynchronizedQueue theInQueue = SynchronizedQueue.createQueue(QueueViewText.class, xPosInQueue,
						yPosInQueue);
				// CREATE THE OUTQUEUE
	 ...
	 * 
	 * Reads the XML file waypoint.xml and calls the create method of WayPoint to
	 * create new WayPoints for the XML
	 * 
	 */
	 ...
	 * create the Autos
	 * 
	 * Reads the XML file autos.xml and calls the create method of Autos to create
	 * new cars for the XML
	 * 
	 */
	private static void createAutos() {
		...
		try {
			...
			// separate every JDOM "object" Element from the list and create Java TheObject
			// objects
			for (Element dasauto : allAutos) {
				...
				for (Element theStation : allStations) {
					...
					for (Station aktuelleStation : Station.getAllStations()) {
						...
						if (theStation.getText().equals(aktuelleStation.getLabel())
	...
	private static void createSteuerLogik() {
		...
		try {
			...
			// creating a new StartStation object
			SteuerLogik.create(label, xPos, yPos, info);// rotPhase, gruenPhase, ampel, wellenZeitPunkt,
														// wellenGenerator);
